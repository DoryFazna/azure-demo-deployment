...
// }
 ...
 resource "azurerm_lb" "mtc-lb" {
  ...
  frontend_ip_configuration {
    name                 = "FrontendIPForAppLoadBalancer"
...
resource "azurerm_public_ip" "pip-web-lb" {
  name                = "PublicIPForLB"
  location            = var.location
  resource_group_name = var.resource_group
 ...
 resource "azurerm_lb" "web-lb" {
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.pip-web-lb.id
  }

}


resource "azurerm_lb_backend_address_pool" "web-bp" {
  loadbalancer_id = azurerm_lb.web-lb.id
  name            = "web-bp"
  depends_on = [
    azurerm_lb.web-lb,
    azurerm_availability_set.web_availabilty_set
  ]
}

resource "azurerm_network_interface_backend_address_pool_association" "web-bp-asso1" {
  network_interface_id    = azurerm_network_interface.web-net-interface.id
  ip_configuration_name   = "web-server"
  backend_address_pool_id = azurerm_lb_backend_address_pool.web-bp.id
}

resource "azurerm_network_interface_backend_address_pool_association" "web-bp-asso2" {
  network_interface_id    = azurerm_network_interface.web-net-interface2.id
  ip_configuration_name   = "web-server"
  backend_address_pool_id = azurerm_lb_backend_address_pool.web-bp.id
}
...
resource "azurerm_lb_rule" "web-lbrule" {
  loadbalancer_id = azurerm_lb.web-lb.id
  name                           = "web-lbrule"
  protocol                       = "Tcp"
  frontend_port                  = 5000
